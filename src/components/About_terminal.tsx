...
const lines = [
  { label: 'Name:', value: personalInfo.name || 'Unknown', className: 'text-green-300' },
  { label: 'Title:', value: personalInfo.title || 'Developer', className: 'text-green-300' },
  { label: 'Shell:', value: 'React + Typescript + Python', className: 'text-green-300' },
  { label: 'Mission:', value: personalInfo.mission || 'Exploring the Matrix', className: 'text-cyan-300' },
  { label: 'Quote:', value: `"${personalInfo.description || 'No description available'}"`, className: 'text-white glitch-text' },
];


  // Initialize ascii animation loop
  useEffect(() => {
...
useEffect(() => {
  ...
  const fullText = lines[currentLineIndex].value;

  const timeout = setTimeout(() => {
    if (charIndex < fullText.length) {
      setTypedLines((prev) => {
        const newLines = [...prev];
        const existing = newLines[currentLineIndex] || '';
        newLines[currentLineIndex] = existing + fullText[charIndex];
        return newLines;
      });
  ...
  return (
    ...
    >
      ...
      <div
        className="md:block hidden w-[650px] min-h-[300px] overflow-hidden"
      >
        <pre className="ascii-glitch text-purple-400 text-xs leading-tight whitespace-pre">
